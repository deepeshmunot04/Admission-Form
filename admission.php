...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_form'])) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Form submitted successfully!'); window.location.href = 'records.php';</script>";
        } else {
...
.form-container:hover {
    box-shadow: 0 12px 24px rgba(255, 99, 71, 0.2); /* Tomato red hover effect */
}
...
.file-input:focus {
    border-color: #ff5722; /* Orange color */
    outline: none;
}
...
.alert {
    margin-top: 20px;
    padding: 15px;
    background-color: #28a745; /* Green for success alert */
    color: white;
    border-radius: 8px;
    text-align: center;
    font-weight: bold;
...
.text-primary {
    color: #007bff; /* Keeping the blue color for primary text */
}
    ...
    <form method="POST" enctype="multipart/form-data">
        <!-- Form Fields -->
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="pin_code" id="pin_code" class="form-control" placeholder="Pin Code" required
                    maxlength="6" pattern="^\d{6}$" title="Pin code must be exactly 6 digits and contain only numbers."
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="mobile" id="mobile" class="form-control" placeholder="Mobile Number" required maxlength="10" oninput="this.value = this.value.replace(/[^0-9]/g, '');" pattern="^[9876]\d{9}$" title="Mobile number must start with 9, 8, 7, or 6 and contain only digits.">
                    <small id="mobileError" class="form-text text-danger" style="display: none;">Mobile number must start with 9, 8, 7, or 6 and contain only digits.</small>
                </div>

                <div class="form-group">
                    <label for="email">Email Id</label>
                    <input type="email" name="email" id="email" class="form-control" placeholder="Email Id" required 
                    pattern="[a-zA-Z0-9._%+-]+@gmail\.com" title="Email must be a valid Gmail address (e.g., example@gmail.com)">
                </div>

                <div class="form-group">
                    <label for="aadhaar">Aadhaar Number</label>
                    <input type="text" name="aadhaar" id="aadhaar" class="form-control" placeholder="Aadhaar Number" required 
                    maxlength="12" pattern="^\d{12}$" title="Aadhaar number must be exactly 12 digits and contain only numbers."
                    oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                </div>
        ...
        <div>
                <p><a href="records.php">Records</a> || <a href="logout.php">Logout</a></p>
        </div>
    </form>

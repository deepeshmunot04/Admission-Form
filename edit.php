...
if (!empty($admission_id)) {
    ...
    $query = "SELECT * FROM admissions WHERE admission_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Loop through files and handle upload
    foreach ($files_to_upload as $file) {
        ...
        if ($_FILES[$file]['name'] != "") {
            ...
            if ($_FILES[$file]['size'] > 1048576 || !in_array($file_extension, ['jpeg', 'jpg', 'png'])) {
                echo "<script>alert('Invalid file for $file! Only JPG, JPEG, PNG files are allowed.');</script>";
                exit;
            }
            ...
            // If no new file is uploaded, set the file path to empty
            $file_paths[$file] = '';
        }
    }
    ...
    // Update query
    $updatequery = "UPDATE admissions SET title=?, first_name=?, middle_name=?, last_name=?, full_name=?, mother_name=?, gender=?, 
    ...
    $stmt->bind_param(
        "sssssssssssssssssssssssss", // 23 string parameters (one for admission_id as well)
        $title, $fname, $mname, $lname, $fullname, $mother_name, $gender, $address,
        $taluka, $district, $pin_code, $state, $mobile, $email, $aadhaar, $dob, $age,
        $religion, $caste_category, $caste, $physically_handicapped, $marksheet_path, $photo_path, $signature_path, $admission_id
    );

    if ($stmt->execute()) {
        echo "<script>alert('Form updated successfully!'); window.location.href = 'records.php';</script>";
    } else {
...
.navbar {
    ...
    padding: 0.5rem 1.2rem; /* Smoother padding */
    position: sticky;
    top: 0;
    z-index: 1000;
...
.form-container:hover {
    box-shadow: 0 12px 24px rgba(0, 0, 0, 0.2); /* Hover effect for the form */
}
    ...
    <select name="title" id="title" class="form-control" required>
        ...
        <option value="Mrx." <?php echo ($row['title'] == 'Mrx.') ? 'selected' : ''; ?>>Mrx.</option>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="pin_code" id="pin_code" class="form-control" placeholder="Pin Code" value="<?= htmlspecialchars($row['pin_code'] ?? '') ?>" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="aadhaar" id="aadhaar" class="form-control" placeholder="Aadhaar Number" value="<?= htmlspecialchars($row['aadhaar'] ?? '') ?>" required>
                </div>
                <div class="form-group">
